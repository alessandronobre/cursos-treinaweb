...
public class CargoAssembler implements SimpleRepresentationModelAssembler<Cargo>{
	...
	public void addLinks(EntityModel<Cargo> resource) {
    		...
    		resource.add(selfLink, editarLink, exclusaoLink);
   
	}

	@Override
	public void addLinks(CollectionModel<EntityModel<Cargo>> resources) {
		Link cadastroLink = linkTo(methodOn(CargoControleApi.class).cadastrar(null)).withSelfRel().withType("POST");
		Link selfLink = linkTo(methodOn(CargoControleApi.class).buscarTodos(null)).withSelfRel().withType("GET");
		resources.add(cadastroLink, selfLink);
	}
